...
// Uses a single ForEach pass to avoid re-looking up every path after sorting.
func (ar *ArchiveReader) ListFiles() []string {
	live := make([]string, 0, ar.index.Len())
	ar.index.ForEach(func(fullPath string, entry FileEntry) {
		if !entry.IsDeleted {
