...
// Satisfies the io.ReaderAt contract: returns io.EOF when the requested
// range extends past the end of data (including reads at exactly end-of-data).
func (m *MemReader) ReadAt(p []byte, off int64) (int, error) {
	...
	n := copy(p, m.data[off:])
	if n < len(p) {
		// Per io.ReaderAt: if fewer bytes are available than requested,
		// return io.EOF (not a custom error).
		return n, io.EOF
	}
	return n, nil
}
